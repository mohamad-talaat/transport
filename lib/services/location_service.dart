...
class LocationService extends GetxService {
  ...
  Future<List<LocationSearchResult>> searchLocation(String query) async {
    try {
      ...
      logger.w('خطأ في البحث عن الموقع: $e');
      return [];
    }
  }

  /// البحث المتقدم باستخدام Nominatim API
  Future<List<LocationSearchResult>> searchLocationAdvanced(
      String query) async {
    try {
      if (query.trim().isEmpty) return [];
      ...
      logger.w('خطأ في البحث المتقدم: $e');
  ...
  /// الحصول على مسار مع نقطة وسطى (Waypoint)
  Future<List<LatLng>> getRouteWithWaypoint(
    LatLng from,
    LatLng waypoint,
    LatLng to,
  ) async {
